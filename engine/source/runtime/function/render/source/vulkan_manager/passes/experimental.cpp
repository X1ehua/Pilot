...
namespace Pilot
{
    ...
    void PExperimentalPass::draw(MeshPerframeStorageBufferObject& m_mesh_perframe_storage_buffer_object)
    {
        ...
        uint32_t perframe_dynamic_offset =
            roundUp(m_p_global_render_resource->_storage_buffer._global_upload_ringbuffers_end[m_command_info._current_frame_index],
                    m_p_global_render_resource->_storage_buffer._min_storage_buffer_offset_alignment);

        m_p_global_render_resource->_storage_buffer._global_upload_ringbuffers_end[m_command_info._current_frame_index]
            = perframe_dynamic_offset + sizeof(MeshPerframeStorageBufferObject);
        assert(m_p_global_render_resource->_storage_buffer._global_upload_ringbuffers_end[m_command_info._current_frame_index]
            <=
            (m_p_global_render_resource->_storage_buffer._global_upload_ringbuffers_begin[m_command_info._current_frame_index]
            +
            m_p_global_render_resource->_storage_buffer._global_upload_ringbuffers_size[m_command_info._current_frame_index]));
